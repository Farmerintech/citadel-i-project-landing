...
const filterBySubject = async (event: React.FormEvent<HTMLFormElement>) => {
  ...
  setLoading(true);
  const theClass = params.class as string;
  const subject = formatSubject(form.subject)
  const year = formatYear(yearIndex);
  const realTerm = formatTerm(form.term);

  console.log(theClass, subject,year,realTerm);
  ...
  try {
    const res = await fetch(
      `https://citadel-i-project.onrender.com/api/v1/class_note/${theClass}/${year}/${realTerm}/${subject}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
type materialItem = {
  ...
  filePath:any
    ...
    return(
        ...
        <option>{theClass}</option>
          </select>
            <select className="flex flex-1  items-center justify-between px-[8px] py-[4px] border-1 border-gray-400 rounded-[4px] w-full md:w-[132px] " name="subject"
            onChange={handleSelect} value={form.subject}>
            ...
            <select className="flex flex-1  items-center justify-between px-[8px] py-[4px] border-1 border-gray-400 rounded-[4px] w-full md:w-[132px] " name="term"
            onChange={handleSelect} value={form.term}>
              <option>Terms</option>
      ...
      <section className="bg-white w-[164px] min-h-screen  rounded-[2px] hidden md:block">
        ...
        <div className="w-[95%] ">
            ...
            {loading && (
              <p className="mt-6 text-center w-full text-[18px] ">Loading material...</p>
            )}
